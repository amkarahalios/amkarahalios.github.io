---
permalink: /
title: "Everything can be optimized"
excerpt: "Everything can be optimized"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Hi - you can call me Anthony. I started my PhD in Fall 2020 at Carnegie Mellon University. I am in the [Algorithms, Combinatorics, and Optimization (ACO) program](https://aco.math.cmu.edu/). I am very fortunate to be advised by the wonderful [Willem-Jan van Hoeve](https://www.cmu.edu/tepper/faculty-and-research/faculty-by-area/profiles/van-hoeve-willem-jan.html). In 2022 I received the NSF GRFP.

Let's talk research interests. For methodologies, I study Decision Diagrams / Dynamic Programming and Integer Programming including cuts and column generation. For applications, I study Vehicle Routing Problems, Graph Coloring, and Scheduling.

My hobbies include [chess](https://www.chess.com/member/anthonykarahalios), [dance](https://www.youtube.com/watch?v=cbIy6a7k4do), [poetry](https://www.poetryfoundation.org/poems/46446/still-i-rise), [athletics](https://runsignup.com/Race/Results/52448#resultSetId-281040;perpage:100), [podcasting](https://www.buzzsprout.com/1890697), and [piano](https://www.youtube.com/user/amkarahalios).

Thanks for visiting! Feel free to reach out!
